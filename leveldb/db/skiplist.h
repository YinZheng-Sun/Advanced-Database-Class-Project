...
class SkipList {
 ...
 public:
  ...
  bool with_hashmap_ = false;

  static std::unordered_map<std::string , Node*> map_;
 ...
 private:
  ...
  Node* Search_map(const char* target) const;
  ...
  //TODO:
  std::string Get_userkey(const char* p);
...
inline void SkipList<Key, Comparator>::Iterator::Seek(const Key& target) {
    if(list_->with_hashmap_) {
        Node* search_res = list_->Search_map((char*)target);
        node_ = search_res == nullptr? list_->FindGreaterOrEqual(target, nullptr) : search_res;
    }
    else
...
void SkipList<Key, Comparator>::Insert(const Key& key) {
  ...
  if(with_hashmap_) {
      std::string userkey = Get_userkey((char*)key);
      auto exist = map_.find(userkey);
      if(exist != map_.end())
          exist -> second = x;
      else
          map_.insert(std::make_pair(userkey,x));
  }
...
template<typename Key, class Comparator>
std::string SkipList<Key, Comparator>::Get_userkey(const char *p) {
    std::string key;
    uint32_t internal_keylen;
    uint32_t keylen;

    p = GetVarint32Ptr(p,p+5,&internal_keylen);
    keylen = internal_keylen - 8;
    key = std::string(p, keylen);
...
template <typename Key, class Comparator>
typename SkipList<Key, Comparator>::Node*
SkipList<Key, Comparator>::Search_map(const char* target) const {
    uint32_t key_length;
    const char * key_t = GetVarint32Ptr((char*)target, (char*)target + 5, &key_length);
    std::string key = std::string(key_t,key_length-8);
...
template<typename Key, class Comparator>
std::unordered_map<std::string , typename SkipList<Key, Comparator>::Node*> SkipList<Key, Comparator>:: map_;
